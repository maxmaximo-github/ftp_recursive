...
def _is_ftp_dir(ftp_handle, name, guess_by_extension=True):
    """
    Simply determines if an item listed on the ftp.

    server is a valid directory or not
    """
    # if the name has a "." in the fourth to last position, its probably a file
    # extension
    ...
    original_cwd = ftp_handle.pwd()     # remember the current working director
    try:
        print(ftp_handle.cwd(name))
        ftp_handle.cwd(name)
        print(ftp_handle.cwd(original_cwd))            # try to set directory to new name
...
def _download_ftp_file(ftp_handle, name, dest, overwrite):
    ...
    Function to download the files of ftp server.
    """

    print(_make_parent_dir(dest.lstrip("/")))
    if not os.path.exists(dest) or overwrite is True:
        try:
            with open(dest, 'wb') as f:
                output = ftp_handle.retrbinary("RETR {0}".format(name), f.write)
                print(output)
...
def download_ftp_tree(
        ftp_handle, path, destination, sleep=4,
        overwrite=False, guess_by_extension=True):
    """
    Download an entire directory tree from an ftp server to the.
    ...
    :param overwrite: set to True to force re-download of all files, even if
     they appear to exist already
    :param guess_by_extension: It takes a while to explicitly check if every
     item is a directory or a file.
        if this flag is set to True, it will assume any file ending with a
        three character extension ".???" is
        a file and not a directory. Set to False if some folders may have
        a "." in their names -4th position.
    """

    # path = ruta_del_servidor
    path = path.lstrip("/")
    # remember working directory before function is executed
    original_directory = os.getcwd()
    # change working directory to ftp mirror directory
    print(destination)
    output = os.chdir(destination)
    print(output)
    files = {}
    # Manejador, dictionario_files, ruta_servidor, guess_by_extension,
...
def main():
    ...
    This function download tree of FTP Server.
    """
    ...
    parser.add_argument('--sleep', type=str, default=4, help=(
        'delay between files to avoid hammering sever (in seconds)'))
